...
def start():
    ...
    pygame.mixer.init()
    titleMusic = pygame.mixer.Sound("Jerry Five.wav")
    startBool = True
    signInBool = True
    name = ""
    width = 600
    ...
    while startBool:
        ...
        blitMessage("V1.0.0", white,purple,display_width*0.96,display_size*0.975,18,False)
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:
...
def blitMessage(msg,colour,shadow_colour,position,y,font_size,select):
    ...
    if select:
        ...
        arrow_width = select_arrow.get_width()
        arrow_height = select_arrow.get_height()
        gameDisplay.blit(select_arrow, [(position-width/2-arrow_width)-16,y-arrow_height/2])
